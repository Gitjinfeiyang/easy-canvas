...
export default class CanvasRender {
  ...
  paint(element) {
    ...
    this._drawBox(element)

    this._drawBackground(element)

    this._drawContent(element)
  ...
  _helpParentRestoreCtx(element) {
    if ((element.isVisible() && !isEndNode(element)) || (!element.isVisible && this.next)) return
  ...
  _drawBox(element) {
    ...
    this._restore(() => {
      this._path(() => {
        ...
        x = element.contentX - element.renderStyles.paddingLeft - borderLeftWidth / 2
        y = element.contentY - element.renderStyles.paddingTop - borderTopWidth / 2
  ...
  _drawBackground(element) {
    const { backgroundColor, contentWidth, contentHeight, paddingLeft, paddingRight, paddingTop, paddingBottom, opacity } = element.renderStyles
