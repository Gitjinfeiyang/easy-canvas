import { isExact, walk, isOuter, parseOuter, walkParent, isEndNode, isAuto, isWX,qSort3,getThrottle } from './utils'
...
function generateRenderTree(element){
  const _root = Object.create(element)
  let preZ = 0
  let pivotZ = 0
  walk(_root,(el) => {
    if(el.hasChildren()){
      el.children = el._getChildren().map(item => Object.create(item))
      // sort by zIndex
      el.children = qSort3(el.children,(pre,pivot) => {
        preZ = pre.renderStyles.zIndex || 0
        pivotZ = pivot.renderStyles.zIndex || 0
        return preZ - pivotZ
      })
      el.children.forEach((item,index) => {
        item.pre = el.children[index-1]
        item.next = el.children[index+1]
      })
    }
  })
  return _root
