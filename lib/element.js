...
import { isExact, walk, isOuter, parseOuter, walkParent, isEndNode, isAuto, findRelativeTo } from './utils'
...
export default class Element extends TreeNode {
  ...
  _initPosition() {
    ...
    if (!this.isInFlow()) {
      // 不在文档流中
      const relativeTo = findRelativeTo(this)
      let { contentX, contentY, contentWidth, contentHeight } = this._getContainerLayout(relativeTo)
