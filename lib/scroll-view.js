...
export default class ScrollView extends View {
  ...
  addEventListener() {
    ...
    this.getLayer().eventManager.addEventListener('click',(e) => {
      if (direction.match('y')) {
    ...
    this.getLayer().eventManager.addEventListener('mousewheel',(e) => {
      if (!this.scrollBy(e.deltaX,e.deltaY)) {
        this.scrollTo({x:this.currentScrollX<=-1000?this.maxScrollX:0,y:this.currentScrollY<=-1000?this.maxScrollY:0})
      }
    },this._scrollView,true)
