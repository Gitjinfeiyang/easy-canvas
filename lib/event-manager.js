...
export default class EventManager {
  ...
  _emit(e) {
     ...
     */
    let callbackList = []
    walk(tree, (node, callContinue, callNext) => {
      if (node.element) {
  ...
  removeElement(element) {
    ...
    this.touchstartList = this.touchstartList.filter(item => {
      if (item.element === element) {
        item.remove()
      }
      return item.element !== element
    })
    this.touchmoveList = this.touchmoveList.filter(item => {
      if (item.element === element) {
        item.remove()
      }
      return item.element !== element
    })
    this.touchendList = this.touchendList.filter(item => {
      if (item.element === element) {
        item.remove()
      }
      return item.element !== element
