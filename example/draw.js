      ...
      h(
        ...
        [
          h(
            ...
            [
              h("view", { styles: { fontSize: 24, color: "#d0916a" } }, [
                h("text", {}, "¥500"),
              ]),
              h("view", { styles: { fontSize: 10, color: "#9a9a9a" } }, [
                h("text", {}, "无门槛使用"),
              ]),
            ]
          ),
          h(
            ...
            [
              h(
                "view",
                { styles: { fontSize: 14, fontWeight: 600, color: "#333" } },
                [h("text", {}, "随机券¥20-300")]
              ),
              h("view", { styles: { fontSize: 11, color: "#9a9a9a" } }, [
                h("text", {}, "领取后30天内可用"),
              ]),
            ]
          ),
...
function drawTable(h) {
  ...
  return [
    h(
      ...
      ["序号", "日期", "姓名", "地区", "城市", "详细地址", "邮编"].map((item) =>
        h("text", { styles: th }, item)
      )
    ),
...
function ontouchstart(e) {
  e.preventDefault();
  layer.eventManager.touchstart(
    e.pageX || e.touches[0].pageX || 0,
    e.pageY || e.touches[0].pageY || 0
  );
}
function ontouchmove(e) {
  e.preventDefault();
  layer.eventManager.touchmove(
    e.pageX || e.touches[0].pageX || 0,
    e.pageY || e.touches[0].pageY || 0
  );
}
